...
import {
  ...
  MentionRef,
  MentionSuggestion,
} from "../components/conversations/chat/ChatMentions";
...
export const useSuggestion = () => {
  ...
  return {
    items: async ({ query }): Promise<MentionSuggestion[]> => {
      const filteredContacts = realm
        .objects<User>(UserSchema.name)
        .filtered("name CONTAINS[c] $0", query) // Filter contacts by the query
        .sorted("name", true);
